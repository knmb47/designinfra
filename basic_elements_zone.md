...
簡単ながらゾーンの境界を置いた絵で示したいと思います。

![物理的な建築物の設計図としての例](basic_element_zone_physical_impl.drawio.png)

この例では、左の図のようにゾーンの程度の順に層構造を作り、一番中心に「屋内で極度にロックされた場所」を配置するのが当然のように考えられます。

真ん中のような「屋外」と「屋内で極度にロックされた場所」を薄い壁一枚で隔てて窓ガラスを付けるとは考えにくいです。ナイトライダー２０００が壁を突き破って突っ込んできて、一気に機械を持っていかれてしまいます。それぞれの「ゾーン」の間には、それなりの「壁」を用意します。一般的には、程度の段階が違うゾーンとの間には、境界すら存在しないようにします。

とは言え、現実には色々と制約がありますから、そうも美しい形にはできません。現実的には、何もない平地に正方形の都を造るような左の図はできないでしょう。右の図のように非機能要件の程度の差がある相手との間は厚い壁を作る等の「境界への機能」を求めるわけです。ここでの壁は機能です。
